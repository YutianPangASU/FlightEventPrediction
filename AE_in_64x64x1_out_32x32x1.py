...
class LoadData:
    def __init__(self, smoothing, horizon):
        ...
        self.X = self.X_density[..., np.newaxis]
...
if __name__ == '__main__':
    ...
    # save path
    save_path = './AE_in_64x64x1_out_32x32x1/Epochs_{}_Horizon_{}_Smooth_{}_nf_{}'.format(epochs, horizon, smoothing, nf)
    ...
    #X_test, Y_test = next(iter(testloader))
    X_test, Y_test = next(iter(trainloader))
    X_test = X_test.view(-1, X_test.shape[-1], X_test.shape[-3], X_test.shape[-2])
    Y_test = Y_test.view(-1, Y_test.shape[-1], Y_test.shape[-3], Y_test.shape[-2])
    if GPU:
