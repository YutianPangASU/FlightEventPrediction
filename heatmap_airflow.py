...
if __name__ == '__main__':
    ...
    for date in range(start_date, end_date + 1):
        try:
            # t_start, t_end = 1564646400 + 86400*(date - start_date), 1564646400 + 3600*8 + 86400*(date - start_date)
            ...
            frames_s1 = np.zeros(shape=(len(timestamp) - 1, resolution, resolution))
            frames_s2 = np.zeros(shape=(len(timestamp) - 1, resolution, resolution))
            frames_s3 = np.zeros(shape=(len(timestamp) - 1, resolution, resolution))
            ...
            for time_idx in range(len(timestamp) - 1):
                ...
                for ax, neighbour in zip(axes.flatten(), neighbours):
                    ...
                    if neighbour == 0:
                        ...
                        if neighbour == smoothing[0]:
                            frames_s1[time_idx, :, :] = im
                        elif neighbour == smoothing[1]:
                            frames_s2[time_idx, :, :] = im
                        elif neighbour == smoothing[2]:
                            frames_s3[time_idx, :, :] = im
                        ax.imshow(im, origin='lower', extent=extent, cmap=cm.Blues)
                        ax.set_title("Smoothing %d neighbours" % neighbour)
                        ax.set_xlim(extent[0], extent[1])
                        ax.set_ylim(extent[2], extent[3])
            ...
            np.save(
                'processed_npy/ac_density_{}_res_{}_tstart_{}_tend_{}_interval_{}_smoothing_{}.npy'
                    .format(date, resolution, t_start, t_end, interval, smoothing[0]), frames_s1)
            np.save(
                'processed_npy/ac_density_{}_res_{}_tstart_{}_tend_{}_interval_{}_smoothing_{}.npy'
                    .format(date, resolution, t_start, t_end, interval, smoothing[1]), frames_s2)
            np.save(
                'processed_npy/ac_density_{}_res_{}_tstart_{}_tend_{}_interval_{}_smoothing_{}.npy'
                    .format(date, resolution, t_start, t_end, interval, smoothing[2]), frames_s3)
