...
class AutoEncoder(nn.Module):  # input res=32, output res=32, nc=2
    def __init__(self, nf):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(nf * 4, nf * 4, 4, 1, 0, bias=False),
            nn.Sigmoid()
        )

        self.decoder = nn.Sequential(
            # input is Z, going into a convolution
            nn.ConvTranspose2d(nf * 4, nf * 8, 4, 1, 0, bias=False),
            nn.BatchNorm2d(nf * 8),
            nn.ReLU(True),
...
if __name__ == '__main__':
    ...
    # save path
    save_path = './AE_in_32x32x2_out_32x32x1/Epochs_{}_Horizon_{}_Smooth_{}_nf_{}'.format(epochs, horizon, smoothing, nf)
