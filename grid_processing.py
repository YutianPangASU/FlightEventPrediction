...
def ev_density_map(ev_file_path, lat, lon, time, neighbours, smoothing, resolution, lon_min, lon_max, lat_min, lat_max,
    ...
    if time[1]-time[0] == 1:  # timeframe 1 second interval
        ...
        frames_s1 = np.zeros(shape=(len(time)-1, resolution, resolution))
        frames_s2 = np.zeros(shape=(len(time) - 1, resolution, resolution))
        frames_s3 = np.zeros(shape=(len(time) - 1, resolution, resolution))
...
def run_EV():
    ...
    for date in range(start_date, end_date + 1):
        try:
            # t_start, t_end = 1564646400 + 3600 * 4 + 86400 * 3, \
            #                  1564646400 + 3600 * 8 + 86400 * 3
            # set parameters
